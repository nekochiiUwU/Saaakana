	...
	if Input.is_action_pressed("Spell2"):
		...
		if ELoad:
			if 0 > ECast:
				...
				ESpell.Launch(Vector2(position.x + 8, position.y - 8), rotation_degrees + rand_range(ELoad / 2, -ELoad / 2), EDamage, 0b11100000000000000000)
				EArrow -= 1
				E = ECD
				ECast = 10
				EShootRelease = 5
	...
	if Input.is_action_pressed("Spell3"):
		if not ROnClick and R <= 0:
			if RCast:
				...
				RSpell.Launch(Vector2(position.x + 8, position.y - 8), rotation_degrees + rand_range(RPrecision / 2, -RPrecision / 2), RDamage, 0b11100000000000000000)
				ROnClick = true
				RShoot = false
				RShootRelease = 5
				R = RCD
